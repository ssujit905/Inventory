...
export default function PrintCenter() {
    ...
    useEffect(() => {
        const fetchSettings = async () => {
            ...
            if (settings) {
                const bName = settings.find(s => s.key === 'business_name')?.value
    ...
    return (
        <DashboardLayout role={profile?.role === 'admin' ? 'admin' : 'staff'}>
            <div className="max-w-5xl mx-auto space-y-6 pb-32">
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                        ...
                        <button
                            ...
                            className="p-3 text-gray-400 hover:text-gray-900 dark:hover:text-white bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl transition-colors shadow-sm"
                    ...
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="pl-10 pr-4 py-3 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-xl text-sm font-bold focus:ring-2 focus:ring-primary outline-none transition-all w-full sm:w-48 shadow-sm"
                            />
                        </div>
                        <button
                            onClick={fetchSales}
                            disabled={loading}
                            className="bg-primary hover:bg-primary/90 disabled:opacity-50 text-white h-12 px-6 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-primary/20"
                        >
                        ...
                        <div className="flex items-center justify-between pb-2 border-b border-gray-100 dark:border-gray-800">
                            <div className="flex items-center gap-4">
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-2 bg-gray-50 dark:bg-gray-800 rounded-lg text-[10px] font-black uppercase tracking-widest text-primary hover:bg-primary/5 transition-colors border border-transparent active:border-primary/20"
                                >
                                    {selectedIds.size === sales.length ? <CheckSquare size={16} /> : <Square size={16} />}
                                    {selectedIds.size === sales.length ? 'Deselect All' : 'Select All'}
                                </button>
                                <h2 className="hidden xs:flex text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest items-center gap-2">
                ...
                {/* Floating Action Bar */}
                {selectedIds.size > 0 && (
                    <div className="fixed bottom-6 left-4 right-4 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border border-gray-100 dark:border-gray-800 p-4 rounded-[2rem] shadow-2xl flex flex-col sm:flex-row items-center gap-4 z-20 animate-in slide-in-from-bottom-8 duration-500 ring-1 ring-black/5">
                        <div className="flex items-center justify-between w-full sm:w-auto px-6 sm:border-r border-gray-100 dark:border-gray-800">
                            <div>
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Queue Status</p>
                                <p className="text-sm font-black text-gray-900 dark:text-white">{selectedIds.size} Ready to Print</p>
                            </div>
                        </div>
                        <button
                            onClick={handlePrintSelected}
                            disabled={printing}
                            className="w-full sm:w-auto bg-primary hover:bg-primary/90 disabled:opacity-50 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-[0.1em] flex items-center justify-center gap-3 transition-all active:scale-95 shadow-xl shadow-primary/20"
                        >
