...
export default function ExpensesPage() {
    ...
    // --- DRAFT PERSISTENCE ---
    useEffect(() => {
        const savedDraft = localStorage.getItem('vendor_expense_draft');
        const savedFormOpen = localStorage.getItem('vendor_expense_form_open');
        ...
        if (savedDraft) {
            ...
            } catch (e) { console.error('Vendor Expenses draft restore failed'); }
        }
    }, []);
    ...
    useEffect(() => {
        if (isFormOpen) {
            ...
            localStorage.setItem('vendor_expense_draft', JSON.stringify(draft));
            localStorage.setItem('vendor_expense_form_open', 'true');
        } else {
            localStorage.removeItem('vendor_expense_form_open');
        }
    }, [description, amount, expenseDate, category, packagingQuantity, isFormOpen]);

    const clearDraft = () => {
        localStorage.removeItem('vendor_expense_draft');
        localStorage.removeItem('vendor_expense_form_open');
    };
    ...
    useRealtimeRefresh(
        () => fetchExpenses(),
        {
            channelName: 'vendor-expenses-changes-v2',
            tables: ['expenses'],
            pollMs: 10000
        }
    );
